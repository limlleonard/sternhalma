...
class Board():
    def __init__(self, centerx, centery, distcc, init_dir=1, nr_spieler=1):
        ...
        self.lst_spieler=self.init_spieler(nr_spieler)
    ...
    def init_spieler(self, nr_spieler):
        ...
        for n1 in range(nr_spieler):
            init_dir=n1 # xx init_direction of a player wählen
            dct1={}
            dct1['lst_piece']=self.init_pieces(centerx, centery, distcc, init_dir)
            dct1['lst_piece_round']=[(round(coord[0]), round(coord[1])) for coord in dct1['lst_piece']]
            dct1['lst_ziel']=self.init_pieces(centerx, centery, distcc, init_dir+3)
            dct1['lst_ziel']=[(round(coord[0]), round(coord[1])) for coord in dct1['lst_ziel']]
            dct1['selected']=None
            dct1['valid_pos']=[]
            dct1['gewonnen']=False
            spieler.append(dct1)
    ...
    def klicken1(self, coord_round):
        spieler=self.lst_spieler[self.order]
        coord_from=coord_to=None
        if coord_round in spieler['lst_piece_round']:
            if spieler['selected'] is not None:
                spieler['valid_pos']=[]
            spieler['valid_pos']=self.find_valid_pos(coord_round) # need to pass in lst_piece_round
            if len(spieler['valid_pos'])>0:
